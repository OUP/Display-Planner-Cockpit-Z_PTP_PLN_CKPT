...
sap.ui.define([], function () {
    ...
    return {
        ...
        dateFormatType: function () {
            ...
            var oDateObj = sap.ui.core.format.DateFormat.getDateInstance(
                {},
                oLocale
            );
            var oLocaleDatePattern = oDateObj.oLocaleData.getDatePattern(
                "medium"
            );
        ...
        formatDate: function (vDate) {
            //formatter to convert ABAP date into Date picker format
            if (vDate) {
        ...
        formatDateRead: function (vDate) {
            //formatter to convert ABAP date into Date picker format
            if (vDate) {
                if (
                    vDate.getDate() == "31" &&
                    vDate.getMonth() == "11" &&
                    vDate.getFullYear() == "9999"
                ) {
                    return "";
                }
                var sysLang = sap.ui.getCore().getConfiguration().getLanguage();
        ...
        formatPurOrg: function (vdesc, vmatnr) {
            if (
                (vmatnr === "" || vmatnr === undefined) &&
                (vdesc === undefined || vmatnr === "")
            ) {
                return;
        ...
        validateTargetValueInput: function (vCtrTyp, vTargetValue) {
            ...
            if (
                (vCtrTyp === "WK" || vCtrTyp === "CWK") &&
                (vTargetValue === "0.000" || vTargetValue === "")
            ) {
                return sap.ui.core.ValueState.Error;
            } else {
                return sap.ui.core.ValueState.None;
        ...
        formatCtrTargetQty: function () {
            // this.oResourceBundle = sap.ca.scfld.md.app.Application.getImpl().getResourceBundle();
            var vQty = this.getOwnerComponent()
                .getModel("i18n")
                .getResourceBundle()
                .getText("TrgtQty");
            var vUnit = this.getOwnerComponent()
                .getModel("i18n")
                .getResourceBundle()
                .getText("OrdUnit");
            return vQty + " / " + vUnit;
        ...
        validateQtyValueInput: function (vCtrTyp, vQtyValue) {
            ...
            if (
                (vCtrTyp === "MK" || vCtrTyp === "CMK") &&
                (vQtyValue === "0.000" || vQtyValue === "")
            ) {
                return sap.ui.core.ValueState.Error;
            } else {
                return sap.ui.core.ValueState.None;
        ...
        formatDocItemCategory: function (vItemCategory) {
            if (vItemCategory === "6") return false;
            else return true;
        },
        formatEditForLimit: function (vItemCategory) {
            if (vItemCategory === "6" || vItemCategory === "A") return false;
            else return true;
        },
        ...
        formatSosInfoCtr: function (
            ...
            if (vInfrvalue) return vInfrtxt + " (" + vInfrvalue + ")";
            else if (vtype === "K") return vCtrtxt + " (" + vCtrvalue + ")";
            else if (vtype === "L") return vschetxt + " (" + vCtrvalue + ")";
        },
        ...
        textFormat: function (text, key) {
            if ((text !== "" && key !== "") && (text !== null && key !== null))  {
